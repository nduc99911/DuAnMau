...
public class TheLoaiDAO  {
    ...
    public List<String> getTenTheLoai(){
        List<String> theLoaiListt=new ArrayList<>();
        Cursor c = db.query(TABLE_NAME,new String[]{"tentheloai"},null,null,null,null,null);
        c.moveToFirst();
        while (c.isAfterLast()==false){
            String tentheloai=c.getString(0);
            theLoaiListt.add(tentheloai);
            c.moveToNext();
        }
        c.close();
        return theLoaiListt;
